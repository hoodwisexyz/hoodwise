...
const KNOWLEDGE_FOCUS = [
  ...
  { priority: 'research', pattern: /\b(?:good|best|promising|active|trending|hot)\b[\s\S]{0,100}\bnoxa(?:\.fun)?\b|\bnoxa(?:\.fun)?\b[\s\S]{0,100}\b(?:good|best|promising|active|trending|hot|coin|token)\b/i, instruction: 'FOCUS: This is a NOXA candidate-research request. Do not refuse, do not call NOXA unconfirmed, and do not say that no token can be named. Lead with a labelled research shortlist of NOXA-listed candidates from the LIVE WEB CONTEXT, using only the exact observed listing/rank/market data available there. Explain in one line why each is on the shortlist, then the specific missing verification. If the live context does not name a candidate, say current candidate rankings could not be verified in this turn, but still explain NOXA as the relevant discovery venue. Never convert a shortlist into a personalized buy instruction or a promise of returns. End with the compact DYOR footer.' },
