...
const SOURCES = [
  ...
  {
    keywords: ['bridge', 'bridging', 'withdraw', 'withdrawal', 'deposit', 'penarikan', 'tarik', 'layerzero', 'stargate', 'ccip'],
...
For candidate-style questions (good/best/promising/hot/trending/which coin): use this direct shape whenever the topic is Robinhood Chain: **Research shortlist** -> 1-3 named candidates or venues supported by current/live/directory evidence; **Why it is on the radar** -> the exact observed reason, not hype; **What would invalidate it** -> contract mismatch, weak liquidity, concentrated holders, privileged controls, or stale listing; **DYOR** -> compact footer. If exact live candidates are unavailable, still name the relevant venue(s) and say the current ranking needs a refresh; do not answer as if the venue is unknown and do not refuse to research.
...
const KNOWLEDGE_FOCUS = [
  ...
  { priority: 'research', pattern: /\b(where|trade|buy|beli|trading)\b[\s\S]{0,100}\b(memecoin|memecoins|meme coin|meme coins|community token|community tokens|token)\b|\b(memecoin|memecoins|meme coin|meme coins|community token|community tokens)\b[\s\S]{0,100}\b(where|trade|buy|beli|trading)\b/i, instruction: 'FOCUS: This is a memecoin venue/trading research request. Name the relevant discovery/trading surfaces such as NOXA, hood.fun, Bankr/Doppler, Uniswap or other DEX routes when supported by context. Do not give a buy instruction. Explain exact contract and pool verification, then end with the compact DYOR footer.' },
  { priority: 'research', pattern: /\b(cashcat|cash cat|\$cashcat)\b/i, instruction: 'FOCUS: This is a Cash Cat / CASHCAT lore and research request. Answer directly: Cash Cat is a named community-token lore candidate tied to Robinhood Chain discovery conversations, not an official Robinhood asset. Explain the meme/thesis first, then say current contract, pool liquidity, holder distribution, ownership controls, and market conditions still need verification. Do not call the topic unknown, undocumented, or unconfirmed; use precise wording like not verified in this context or needs a live refresh. End with the compact DYOR footer for any good/bad or buy-adjacent question.' },
  { priority: 'research', pattern: /\b(hood\.fun|hoodfun|hood fun)\b[\s\S]{0,100}\b(pick|recommend|good|best|hot|trending|bagus|pilih|coin|token)\b|\b(pick|recommend|good|best|hot|trending|bagus|pilih)\b[\s\S]{0,100}\b(hood\.fun|hoodfun|hood fun)\b/i, instruction: 'FOCUS: This is a HoodFun candidate-research request. Do not answer with a generic refusal and do not say no candidate can be named. Lead with HoodFun as the relevant Robinhood Chain community launchpad/discovery surface. If live candidate metrics are available, name the observed candidates and the exact evidence. If live metrics are not available, say the current listing needs a refresh and give the precise research path: open HoodFun current listings, copy the exact token contract, verify it on Blockscout, then inspect the DEX pool/liquidity and ownership controls. End with the compact DYOR footer.' },
  ...
  { pattern: /\b(bridge|bridging|withdraw|withdrawal|deposit|penarikan|tarik|layerzero|ccip|stargate)\b/i, instruction: 'FOCUS: Explain route, timing, trust model, fees, and the next onchain step. If the user asks about withdrawing from Robinhood Chain to Ethereum, answer the canonical bridge timing directly: documented withdrawals can take about 7 days because of the challenge period. Do not pivot to testnet availability unless the user specifically asks about testnet.' },
