...
const CANDIDATE_QUESTION_PATTERN = /\b(good|best|promising|active|trending|hot|which|what coin|coin apa|token apa|bagus|pilih|recommend|pick|shortlist)\b/i;
const EVASIVE_RESEARCH_PATTERN = /\b(cannot recommend|can't recommend|cannot name|can't name|no specific token|unknown platform|undocumented|not documented in available|unconfirmed and no live evidence|any recommendation would be fabricated)\b/i;
...
function reviewAnswer({ question = '', answer = '', sources = [], usedLiveSearch = false } = {}) {
  ...
  const needsResearch = RESEARCH_PATTERN.test(questionText);
  const needsCandidateShape = needsResearch && CANDIDATE_QUESTION_PATTERN.test(questionText);
  const hasDirectOpening = trimmed.split(/(?<=[.!?])\s+/)[0]?.length >= 18;
  const hasSourceSupport = sources.length > 0 || !usedLiveSearch;
  const hasResearchShape = !needsResearch || /research snapshot|research shortlist|thesis|lore|dyor|contract|liquidity|candidate|discovery/i.test(trimmed);
  const hasCandidateShape = !needsCandidateShape || (/research shortlist|candidate|on the radar|discovery/i.test(trimmed) && /DYOR|do your own research/i.test(trimmed));
  ...
  if (!hasCandidateShape) reasons.push('missing_candidate_shape');
  ...
  const checks = [hasDirectOpening, hasSourceSupport, hasResearchShape, hasCandidateShape, researchDirect, privacySafe, sourceSafe];
...
function shouldRepairAnswer(review) {
  ...
  if (review.reasons.includes('missing_candidate_shape')) return true;
...
function buildRepairPrompt({ question = '', answer = '', sources = [] } = {}) {
  ...
  return [
    ...
    '- If the user asks for memecoins, launchpads, lore, thesis, or candidates, provide an evidence-led research shortlist/snapshot with concrete names when available from context, explain why each is on the radar, then add a compact DYOR warning.',
