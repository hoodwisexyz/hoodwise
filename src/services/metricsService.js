...
const counters = { chatRequests: 0, chatErrors: 0, streamFallbacks: 0, liveSearches: 0, verifierRequests: 0, verifierErrors: 0 };
const timings = { chat: [] };
...
function recordRequest(path, status, durationMs) { if (path === '/chat' || path === '/chat/stream') { record('chatRequests'); record('chat', durationMs); if (status >= 400) record('chatErrors'); } if (path === '/contracts/verify') { record('verifierRequests'); if (status >= 400) record('verifierErrors'); } }
